...
def normalize(table, column):
    ...
    Args:
        ...
        column: The column label in the table to normalize.

    """
    min_num = min(column_values(table, column))
    max_num = max(column_values(table, column))

    for row in table:
        row[column] = (row[column] - min_num) / (max_num - min_num)
