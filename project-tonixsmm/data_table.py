...
class DataRow:
    ...
    def compare(self, other, columns):
        """Compare two DataRow objects based on the given columns.

        Args:
            other (DataRow): The other DataRow object to compare
        ...
        Note:
            Two DataRow objects must have identical columns
        """
        # Error checking
        if not isinstance(other, DataRow):
            raise ValueError('expecting DataRow object')
        if self.columns() != other.columns():
            raise ValueError('two rows must have identical columns')
...
class DataTable:
    ...
    def __init__(self, columns=[], data=None):
        ...
        if data is not None:
            for row in data:
                self.__row_data.append(DataRow(columns=self.columns(), values=row))
    ...
    def count(self, column, value):
        """Returns the number of rows in the table with the given value
        in the given column.
    ...
    @staticmethod
    def concat(table1, table2, axis=0):
        """Returns a new data table holding the result of concatenating table 1 and 2.

        Args:
            table1: First data table to be combined.
            table2: Second data table to be combined.
            axis: The axis to concatenate along. 0 for row, 1 for column

        Notes:
            Column names do not need to match.
            Duplicate column names removed from table2 portion of result.
            If concatenate along row, table1 and table2 must have the same number of columns.
            If concatenate along column, table1 length must be greater or equal to table2 length.
            Similar to Pandas concat() - kind of

        """
        # Error checking
        if axis != 0 and axis != 1:
            raise ValueError('bad axis')
        if not isinstance(table1, DataTable) or not isinstance(table2, DataTable):
            raise ValueError('expecting DataTable object')
        if axis ==0 and table1.column_count() != table2.column_count():
            raise ValueError('table1 and table2 must have the same number of columns')
        if axis == 1 and not table1.column_count() >= table2.column_count():
            raise ValueError('table1 must have more or equal number of columns than table2')
        
        # Create a new table
        new_col = table1.columns()

        # Concatenate algorithm
        if axis == 0:
            new_table = DataTable(new_col)
            for row in range(table1.row_count()):
                new_table.append(table1[row].values())
            for row in table2:
                new_table.append(table2[row].values())

        elif axis == 1:
            new_col += ([item for item in table2.columns() if item not in new_col])
            new_table = DataTable(new_col)
            for row in range(table1.row_count()):
                try:
                    new_row = table1[row].values() + table2[row].values()
                except:
                    new_row = table1[row].values() + ['' for item in table2.columns()]
